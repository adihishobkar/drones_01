...
class safe_teleop:
    def __init__(self):
        ...
        self.min_vel_y = -1
        self.max_vec_y = 1
        self.drive_scale = 0.3 # scaling factor to scale the net force
...
#-------------------------------------------------
    ...
    def compute_motion_cmd(self):
        if (self.obstacle_vector is None):
            ...
            cmd.linear.x = np.clip(vel_x,self.min_vel_x,self.max_vec_x) * mag
            cmd.linear.y = np.clip(vel_y,self.min_vel_y,self.max_vec_y) * mag
